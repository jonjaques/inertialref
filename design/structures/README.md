...
scene. Its `Mars pad` collection holds 591 separate parts. The `Preview only`
...
uses ten materials and no textures. The export is 29,184 triangles and
1,573,464 bytes, below the enforced budgets of 35,000 triangles and twelve
material batches.

The game reads meters with +Y up. The clear landing circle has a 25 m radius,
...
extends 57 m along +Z in glTF, leaving the outer apron's corner flush with
its top, and the highest service unit is below 2.1 m.
Hidden slab tops do not overlap the visible deck or apron. Hazard sectors
have geometric gaps, since layered millimeter decals share depth bins in

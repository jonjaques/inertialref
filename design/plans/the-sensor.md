...
plan makes and phase 0 measures. A budget is not a measurement, and the perf
...
Boot learns the chain. `PostProcessing` builds its quad material on the first
`render()`, and a pipeline compiled on the first presented frame is exactly the
hitch the warm-up census exists to hide, so `render/warmup.ts` gains a producer
for the chain's materials, registered by label like every other one.
...
**Phase 0 — the spine.** `PostProcessing` with the scene pass and the house
curve, no effects, `Sensor.tsx` at priority 1, warm-up registered, the pass
sized by R3F's own DPR. Gate: a `--shot` at headroom 1 is pixel-identical to the
current frame within 1/255; a `vec4f(8, 4, 2, 1)` written through the chain
survives to the extended canvas, as in spike 1; the chain's cost by
`measureGpuFrameMs` at three operating points — flight start, planetarium at
Earth from 14,400 km, the Earth summit stance — recorded here (budget: the
spine adds **0.15 ms** at 1080p). The watchdog and the drive rig still see a
presented frame.
...
frame at each operating point is not in the perf plan and phase 0 measures it
...
1. **Phase 0 and phase 1 together.** Nothing else on this page is honest until
   the exposure is, and the galaxy plan is waiting on it.
...
- **Every millisecond above is a budget until phase 0.** The M5 is well above
  the target laptop, and the perf plan's rig is 1600×900; a 1080p figure on the
  target is what the table wants and nothing here has one yet.
...
```bash
# the chain's cost at an operating point. `measureGpuFrameMs` is the perf
# dock's "measure gpu" button today; phase 0 exposes it as `ir.gpu()` so the
# driver can take the figure without a click.
node scripts/drive.mjs --url 'http://localhost:5173/?timing=full' \
  --js "await ir.gpu(40)" --down

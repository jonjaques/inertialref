...
- **Render compression is radial about the eye, never about the origin.** `placeAt` and
  ...
  every caller outside it has to be given the same one. Orbit traces use the same
  scene eye to resolve physical occlusion before placing their visible directions. The origin is

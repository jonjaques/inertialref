...
| Terrain patch generation                       | ≤ 8 ms per patch per worker           | **20 ms** airless, **37 ms** atmosphered, bordered — measured 2026-08-28 on an M5, Node 26, main thread, with the band stack. **Over budget by 150–360%.** The three bands it replaced re-measure at 3.6–3.8 ms on the same machine, so the geology is five to ten times them and under three times the 12.8 ms recorded on 27 Aug; flattening the gradient table paid for the difference. The crater ladder is two thirds of it |

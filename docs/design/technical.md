...
| Terrain patch generation                       | ≤ 8 ms per patch per worker           | **9.5 ms** with no craters, **32 ms** rocky airless, **34 ms** Iapetus, **38 ms** rocky atmosphered, bordered — measured 2026-08-29 on an M5, Node 26, main thread, with the band stack. **Over budget by 19–378%.** The three bands it replaced re-measure at 3.6–3.8 ms on the same machine. The crater neighborhood is most of it and nearly all of the spread: it walks about five cells an axis, which is what it costs to contain the ejecta reach and the radial slop of a cube lattice cut by a shell, so a world with no craters keeps the whole budget saving. The atmosphered world is no longer the expensive one — writing `pcg3d`'s first lane out inside `gradientAt` took it from 60 ms, because the analytic-gradient path its erosion damping reads was allocating an object per lattice corner |
...
| Area           | Work                                                                                                                                | Seam                                                                                                                                     |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Terrain**    | Geology, biomes and materials; scatter                                                                                              | Quadtree LOD, geomorphing, bordered patches and cross-face sampling landed with [ADR-0015](../adr/0015-terrain-level-of-detail.md)       |

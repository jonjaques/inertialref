...
| Runtime               | Node 26, pnpm 12; Node runs the TypeScript sources directly                                                                                                                    |
| Bundle                | 2.49 MB, **736.0 KB gzip / 555.7 KB brotli**, dominated by Three.js, **no code splitting**                                                                                     |
| Simulation throughput | ~1.25M ticks/s in-browser for one integrated entity; a coasting one is on rails and a frame jumps its ticks (ADR-0025)                                                         |
| Offline               | Service worker + IndexedDB + a migration chain, verified with the server stopped                                                                                               |
| Gate                  | `pnpm check` — every stage as one graph: graph, spelling, brand, presets, format, lint, typecheck, test, test:slow, the capability self-test, build. CI runs the same command. |
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Compute shaders**                | Terrain generation, quadtree selection and geomorphing on the GPU. Heightfields are a TSL compute kernel, sixteen tiles a dispatch, with the worker pool as canon and fallback (ADR-0023); selection and the mesh are still CPU-side. |
...
instanced, and it is the only thing that is. The atmosphere is an analytic
shell awaiting its LUTs.
...
| Terrain reconciliation    | 1.0 ms | Selection 0.11–0.31 ms, mesh build 0.25 ms a patch at eight a frame — 2 ms, twice this line, for the seconds a convergence takes and nothing once it has; heightfields come from the GPU producer (ADR-0023), the worker pool as fallback. Measured 2026-08-27: the whole frame is **2.04 ms at 63.9 fps** standing on Miranda's summit, 438 patches and 3.59 M triangles selected |
...
The measurements below retain their original rigs and dates. They are
historical operating points unless a newer date is named, not a fresh whole-app
benchmark. Galaxy allocations and measured limits are in
[ADR-0038](../adr/0038-the-stars-and-the-diffuse-sky.md); startup and shell
ownership are in [ADR-0039](../adr/0039-the-shell-before-the-scene.md).

> **Where these numbers come from now.** The dev dock's **perf** panel (`P`) plots
...
| Client bundle, gzipped                         | ≤ 900 KB with code splitting          | **747.0 KB gzip / 583.8 KB brotli** for the entry chunk, measured 2026-08-27 — was 736.0 KB on 25 Aug. The only split is the documentation's Mermaid import, 116 chunks nobody fetches until they open a page with a diagram                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Catalog, 150 ly, over the wire                 | Was a guess at ~2 MB                  | **178 KB brotli**, measured — [spike 3](../spikes.md#3--catalog-bundle-size); the naked-eye sky beyond it is a second file at **188 KB brotli**, 366 KB for the pair                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Material sets, per biome                       | ≤ 12 MB                               | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Peak JS heap                                   | ≤ 900 MB                              | **66–74 MB** across orbit, approach and surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Terrain patch generation                       | ≤ 8 ms per patch per worker           | **9.8 ms** with no craters, **35 ms** rocky airless, **38 ms** Iapetus, **41 ms** rocky atmosphered, bordered — measured 2026-08-29 on an M5, Node 26, main thread, with the band stack and the surface cover, as the median of three consecutive runs with nothing else on the machine. A reading taken with Chrome and a dev server up is two to three times these and says nothing. **Over budget by 23–413%.** The three bands it replaced re-measure at 3.6–3.8 ms on the same machine. The crater neighborhood is most of it and nearly all of the spread: it walks about five cells an axis, which is what it costs to contain the ejecta reach and the radial slop of a cube lattice cut by a shell, so a world with no craters keeps the whole budget saving. The atmosphered world is no longer the expensive one — `gradientAt` hashes with `noise3`'s `hash3`, written out rather than called, where it hashed with `pcg3d` and discarded two of three lanes: 60 ms became 37. The GPU producer makes this the fallback's figure: sixteen tiles in **10.0 ms** on the same M5's GPU against 805.6 ms for the same sixteen on the CPU, and a converged 2 m stance on Luna in 4.4 s against 25–33 s from the pool (ADR-0023) |
| Worker queue latency, p95                      | ≤ 40 ms                               | Instrumented and measured during terrain and survey contention; no universal p95 bound established                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Save size                                      | ≤ 4 KB                                | 1,041-byte self-test fixture on 9 Sep 2026, including its coasting epoch; the 4 KB target is not a bound for arbitrary sessions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Surface maps and shape models, over the wire   | —                                     | **26 MB**: 25 MB of maps and 1.0 MB of shape models, content-hashed, fetched at boot behind the overlay — the 14 MB hull is fetched there too                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Draw calls                                     | ≤ 1,200                               | **10–17** measured on 20 Aug, when the scene was 29 bodies. Sol is 129 now and most of them cull, but the number has not been re-measured with the belt in frame                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
> the first afternoon. That plot is the perf panel's warp row now, and the seven
> detents are seven speeds: the ceiling is on integration, and a coasting ship
> has none to be capped by ([ADR-0025](../adr/0025-the-rails.md)).
...
| **No filesystem**                                         | Saves are IndexedDB; export is a download                                                                                                                             | Already handled; the harness exposes serialized saves; a player-facing file workflow remains design scope                                                                                                                                                                                                                                                                |
| **Shader compilation stalls**                             | A first-frame hitch when entering a new visual state                                                                                                                  | **Built** (23 Aug 2026) — boot warms every archetype pipeline behind the loading overlay (`render/preload.ts`), and the real instances build ahead of need: one body per frame in `Bodies.tsx`, `WarpFx`/`TerrainPatches` at mount. CONTEXT.md § "The first look was the expensive one" has the measurements, including why an archetype twin does not cover an instance |
...
| Area           | Work                                                                                                                                | Seam                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Terrain**    | Authored material sets, erosion, scatter past boulders                                                                              | Quadtree LOD, geomorphing and cross-face sampling landed with [ADR-0015](../adr/0015-terrain-level-of-detail.md); the geology with [ADR-0019](../adr/0019-the-geology.md), the material with [ADR-0020](../adr/0020-the-face.md), the meter scale and the rocks with [ADR-0021](../adr/0021-the-ground.md), and the sea, the coast and the sphere's own bake with [ADR-0026](../adr/0026-the-liquid.md) — the detail field in `render/terrain.ts` is where hex-tiling and triplanar go |
...
| **Streaming**  | A spatial index for interest, and a simulated-but-not-rendered tier                                                                 | `updateInterest`; `systemsWithin` already bounds and refuses oversized queries. Predictive loading and a per-frame generation budget landed with the terrain quadtree                                                                                                                                                                                                                                                                                                                  |
...
| **Rendering**  | [Photometric normalization](../roadmap.md#photometric-normalization) — scale each surface map toward its published geometric albedo | `PlanetMaterial.albedoScale` already exists and is already driven per body; the ingest already decodes every map and can record its mean                                                                                                                                                                                                                                                                                                                                               |
...
| TypeDoc, marked, shiki, github-slugger       | Tooling              | The documentation build only — `scripts/docs/`, never imported by anything that ships                                                                                             |
| Mermaid                                      | `apps/game` only     | Dynamically imported by the documentation mode, so it is 116 chunks a reader fetches on the first page with a diagram and the application never touches                           |

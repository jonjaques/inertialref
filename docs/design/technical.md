...
| Terrain patch generation                       | ≤ 8 ms per patch per worker           | **14.5 ms** bordered, 12.8 unbordered, 0.33 M samples/s — measured 2026-08-27 on an M5, Node 26, main thread. **Over budget by 80%**, before any geology. A _coarse_ patch is dearer: 20.7 ms at level 1 against 14.3 at level 12, because consecutive samples land in different noise lattice cells |

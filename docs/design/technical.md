...
| Terrain patch generation                       | ≤ 8 ms per patch per worker           | **9 ms** with no craters, **32 ms** rocky airless, **34 ms** Iapetus, **60 ms** rocky atmosphered, bordered — measured 2026-08-29 on an M5, Node 26, main thread, with the band stack. **Over budget by 17–650%.** The three bands it replaced re-measure at 3.6–3.8 ms on the same machine. The crater neighborhood is most of it and most of the spread: it walks about five cells an axis, which is what it costs to contain the ejecta reach and the radial slop of a cube lattice cut by a shell, so a world with no craters keeps the whole budget saving and an atmosphered one pays twice over — its erosion damping is what reads the analytic gradient |

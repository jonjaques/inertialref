...
| Ships                    | 🟡     | Two modeled hulls in `data/models/` (a CC-BY Enterprise-D and a CC-BY Rocinante, each scaled to its true length), chosen in Display settings, debug cone as fallback; no subsystems                                                                                                                |
...
| Structures, settlements  | 🟡     | Durable body-fixed placements and the Mars landing pad exist, with a declared walkable relief for contact — deck, aprons, ramp, equipment as walls. Settlements and interiors remain unbuilt. [ADR-0040](adr/0040-structures-keep-a-body-fixed-anchor.md)                                          |
| Humanoids                | 🟡     | Body-fixed characters walk, sprint, strafe, crouch, and jump; optional pointer lock drives first-person and third-person controls. Trusted session permission gates flight. Suit survival and object interaction remain unbuilt. [ADR-0047](adr/0047-the-character-walks-in-a-body-fixed-frame.md) |
...
exploration is 🟡. A character can traverse terrain and a structure's relief,
with a choice of first-person or third-person controls. Rock and hull contact,
interior movement, survey tools, inventory, and suit hazards remain open.
See [On foot](design/onfoot.md) for current controls and limits.
...
[ADR-0020](adr/0020-the-face.md) is the decision record. The cover — four bytes then, eight since the liquid — carries what a shader cannot derive — impact-fresh material, flood basalt, where
...
| Vertex attributes are float32             | 270 KB a patch — 203 KB of float32 geometry and 68 KB of cover — so a whole-disk selection is 129–291 MB at the flight lens                         | Int8 normals and Int16 morph deltas are worth about half                                                                                                          |
...
| ~~The cold descent on every CI run~~ ✅ | The workflow restores the terrain archive from a cache keyed on its version string and saves it when it grew; the run on `main` is what leaves a warm archive for every pull request                                                                                                                                                                                                                                                                                                                                                                                                     |
| No image comparison in CI               | `pnpm presets:compare` differences the lit pictures against their plates or a baseline commit, on a machine with a GPU; the headless picture ledger runs in `pnpm test` everywhere. A hosted GPU runner is the same open question as the shader suite below                                                                                                                                                                                                                                                                                                                              |
...
| Terrain and declared-relief contact only      | Character stepping, wall sliding, uphill slope limits, and local-gravity jumps exist. Rocks, hull walls, ceilings, arbitrary meshes, and entity-to-entity contact do not.                                                                                                          |
| Surface characters are kinematic              | Tangent movement and local-gravity jumps run in the body-fixed frame. No orbital Coriolis or centrifugal integration, moving ship interiors, or suit survival.                                                                                                                     |

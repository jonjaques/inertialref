...
| Patch generation is over its budget              | 9 to 38 ms per bordered 65×65 patch across the zoo — 9 with no craters, 32 rocky airless, 34 icy dead, 38 rocky atmosphered — against a documented ≤ 8 ms | `pnpm sim --terrain-baseline` is the measurement; the crater neighborhood is most of it, and its radial bound, `EJECTA_REACH` and the GPU producer are the levers |

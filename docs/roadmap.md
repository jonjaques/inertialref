...
| Patch generation is over its budget              | 20 ms per bordered 65×65 patch on an airless world and 37 on an atmosphered one, against a documented ≤ 8 ms | `pnpm sim --terrain-baseline` is the measurement; the crater ladder's depth and the GPU producer are the levers    |

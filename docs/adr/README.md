...
| [0029](0029-the-sensor-spine.md)            | The sensor spine            | accepted     | The sensor owns the frame for everyone: one `PostProcessing` around the scene pass and the house curve, MSAA on the pass and the canvas single-sampled; the pass is keyed on the render call rather than three's own frame counter, so a measured frame is a frame; the chain restores the renderer around the quad, because `PostProcessing.render` swaps it and a throw from the scene left the picture one transfer too dark for good; and three's backend is patched to draw nothing for a pipeline the warm-up is still building. Level with the baseline at five operating points, two of them at Proxima Centauri d. |

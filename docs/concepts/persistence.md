...
> address to regenerate from. The self-test's flown session serializes to
> 1,041 bytes in the 9 Sep 2026 check. That is an example, not a size limit:
> entities, loaded-system references and mutations can grow. A coasting ship's
> epoch is stored because its propagation origin is canonical too
...
```mermaid
flowchart LR
    BASE["<b>deterministic base universe</b><br/>seed + address + algorithms + catalog<br/><i>free, infinite, identical everywhere</i>"]
...
The budget checks that fixture, not an arbitrary universe with more entities.
...
### Generation drift is reported separately from schema validity

A valid save can still name content generated by different algorithms or a
different catalog. `restoreSave` returns that drift on success, so the host can
explain the changed inputs; a reference that cannot be regenerated fails the
restore. Compatibility does not come from freezing generated planets into the
file. Portable pictures have a different, stricter algorithm check and do not
yet carry a catalog version. See [ADR-0033](../adr/0033-presets-hold-a-photographic-instant.md).
...
blocks the main thread. A small current save does not justify a storage port
that becomes unsuitable when mutations arrive.
...
The service worker in `apps/game/public/sw.js` caches each kind of resource
according to its identity:

| Resource                                                               | Policy                                                                               |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Navigation HTML                                                        | Network-first, with the requested route's own cached document as the outage fallback |
| Hashed `/assets/` files                                                | Cache-first; the previous build's cache remains available for matching hashed files  |
| `/media/` files                                                        | Cache-first within one build                                                         |
| Other public files                                                     | Stale-while-revalidate                                                               |
| Live API/WebSocket traffic, range requests and explicit cache bypasses | Network only                                                                         |

An uncached route cannot borrow the menu's HTML: it would hydrate the wrong
page. Offline navigation without its own cached document returns 503. Cache
writes are best effort, so storage denial cannot turn a successful online
response into a failure.

Generation needs no content server, but the client also reads vendored
catalogs, models, textures and documentation. Those assets must have reached
the cache before they are available offline. A warmed production build has
passed all twelve capability checks with its server stopped; that does not
prove every unvisited page or model is cached. The test's scope is described in
[testing](../guides/testing.md).

Saves and preferences belong to the browser origin. The two production domains
have separate storage; visiting one does not migrate the other's saves.
[Hosting](../hosting.md) describes the deployment boundary.

Finished physical sky cubes have a separate, bounded IndexedDB cache. They are
regenerable rendering data and never enter a save. Missing or incompatible
entries trigger recomputation. [ADR-0038](../adr/0038-the-stars-and-the-diffuse-sky.md)
defines their fingerprint and lifetime.

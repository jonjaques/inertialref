...
optimization, they are a requirement.** The spike was written to ask whether TSL
...
volume-complete catalog; by 150 ly it is a magnitude-limited one wearing the
same shape.
...
| ------------ | ------------------------------------------------------------------------------------------------------------------- |
| **Blocks**   | [M4](design/production.md#m4--the-explorer--mvp)                                                                    |
| **Question** | What exactly do the licenses require, and where must the attribution appear?                                        |
...
The bible recorded Gaia as _"Open with attribution"_. ESA's own license page says
otherwise, verbatim:
...
> detailed request for authorization/license shall be made by the User by sending
> email to data.licenses@esa.int.
...
argues at length that a non-commercial clause is not an open source license and
that the project deliberately avoided one. Bundling Gaia would attach exactly that
restriction to the shipped artifact — not to the Apache-2.0 code, but to the data
...
reads much more permissively than the license page. **A documentation page and a
license page that disagree are a reason to take the stricter one**, and to ask ESA
...
| **Gaia — full acknowledgment** | "This work has made use of data from the European Space Agency (ESA) mission Gaia (https://www.cosmos.esa.int/gaia), processed by the Gaia Data Processing and Analysis Consortium (DPAC, https://www.cosmos.esa.int/web/gaia/dpac/consortium). Funding for the DPAC has been provided by national institutions, in particular the institutions participating in the Gaia Multilateral Agreement." |
| **HYG**                        | CC BY-SA 4.0 (v4.x; v3.x was CC BY-SA 2.5). Credit _The HYG Database, astronexus_, link the license, link the source, and state that it was modified.                                                                                                                                                                                                                                              |
...
So the packed catalog from [spike 3](#3--catalog-bundle-size) **is Adapted
...
on `packages/*` and CC BY-SA 4.0 on the catalog coexist without conflict,
because they cover different works.

**That has a concrete engineering consequence.** The packed catalog must ship as
its own asset with its own license notice beside it — _not_ inlined into the JS
bundle as a literal, which would blur exactly the boundary the licenses depend on.
...
notice, a notice referring to the license, a notice referring to the warranty
...
archive's own pages state no license at all.** It is operated by Caltech under
...
EU. Treat the requested acknowledgment as the obligation, use it, and stop
...
1. **Ship HYG + NASA. Keep Gaia out of the bundle** until ESA answers a written
   request at `data.licenses@esa.int`. HYG's CC BY-SA is share-alike, which is an
...
3. **`NOTICE` becomes required at the first ingest**, and the license text plus
   all three attribution strings go in it in the same change that first reads a
   dataset.
4. **In-product attribution on the catalog panel**, as the design already
...
HYG's pre-merged cross-catalog identity resolution. HYG is the license-clean
...
| Axes normalized from the element's own logical min/max at 8/16/32-bit report size | `gamepad_device_mac.mm`                                                    |
...
Axis resolution is _not_ a problem: normalization uses the device's own logical

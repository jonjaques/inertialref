...
export function uploadStarfieldAppearance(
  material: Pick<StarfieldMaterial, 'colors' | 'enabled'>,
  stars: StarField,
  ...
  const colors = material.colors.array as Float32Array
  let first = count
  let last = -1
  ...
  for (let i = 0; i < (remap ? count : span); i++) {
    ...
    const before = previous?.colors[i]
    if (
      i < oldCount &&
      (color === before ||
        (color !== undefined &&
    ...
    colors[i * 3] = red * normalization
    colors[i * 3 + 1] = green * normalization
    colors[i * 3 + 2] = blue * normalization
    first = Math.min(first, i)
    last = i
  }
  if (first <= last) {
    material.colors.addUpdateRange(first * 3, (last - first + 1) * 3)

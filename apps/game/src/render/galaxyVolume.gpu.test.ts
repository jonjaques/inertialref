...
import { DEFAULT_PICTURE, type Picture } from './picture.ts'
...
it.each([
  DEFAULT_PICTURE,
  { aa: 'temporal', scale: 'quality', sharpness: 'off' } satisfies Picture,
])(
  'composes foreground depth and color in $aa at $scale resolution',
  async (picture) => {
    ...
    const foreground = uniform(0)
    material.colorNode = vec3(foreground, 0, 0)
    ...
    try {
      ...
      if (picture.aa !== 'temporal')
        expect(covered.at(20, 32)[0]).toBeGreaterThan(clear.at(20, 32)[0] * 0.9)
      else {
        expect(covered.data.every(Number.isFinite)).toBe(true)
        expect(covered.data.some((v, i) => i % 4 === 0 && v > 0.1)).toBe(true)
      }
      sensor.render(output)
      const repeated = await gpu.read(output)
      if (picture.aa !== 'temporal')
        expect(repeated.at(20, 32)).toEqual(covered.at(20, 32))
      else expect(repeated.data.every(Number.isFinite)).toBe(true)
      ...
      foreground.value = 0.2
      for (let i = 0; i < 4; i += 1) sensor.render(output)
      const lit = await gpu.read(output)
      expect(lit.at(32, 32)[0]).toBeGreaterThan(0.1)
      expect(lit.at(32, 32)[0]).toBeGreaterThan(lit.at(32, 32)[2] * 3)
      foreground.value = 0

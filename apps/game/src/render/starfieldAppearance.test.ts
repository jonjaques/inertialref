...
it('retains appearance buffers and updates duplicate names, shrinking selections, and resolved disks', () => {
  const material = {
    colors: new InstancedBufferAttribute(new Float32Array(100_000 * 3), 3),
  ...
  expect(material.colors.updateRanges).toEqual([{ start: 0, count: 9 }])
  const version = material.colors.version
  material.colors.clearUpdateRanges()
  ...
  expect(material.colors.version).toBe(version)
  expect(material.colors.updateRanges).toEqual([])
  ...
  expect(material.colors.updateRanges).toEqual([{ start: 0, count: 3 }])
})
